 ...
 * from mumuy and thanks very much!
 * extended by zhongchao on 2017/11/07
 */
;(function (factory) {
    ...
    $.support.cors = true;
    $.fn.city = function(parameter,getApi) {
        ...
        var defaults = {
            ...
            dataUrl: $('#js_citys').attr('data_src'),     //数据库地址
            extras: $('#js_citys').attr('extras'),        //扩展配置地址
                    ...
                    $.ajax({
                        ...
                        success:function(data){
                            ...
                            if(options.code){   //如果设置地区编码，则忽略单独设置的信息
                                ...
                                if (extras && extras.deactive && extras.deactive.length) {
                                    if ($.inArray(String(options.province), extras.deactive)>-1) {
                                        options.province = 0;
                                        options.city = 0;
                                        options.area = 0;
                                    }
                                    if ($.inArray(String(options.city), extras.deactive)>-1) {
                                        options.city = 0;
                                        options.area = 0;
                                    }
                                    if ($.inArray(String(options.area), extras.deactive)>-1) {
                                        options.area = 0;
                                    }
                                }
                            }
                            var updateData = function(){
                                ...
                                for(var code in data){
                                    if(!(code%1e4)){     //获取所有的省级行政单位
                                        ...
                                        if (extras && extras.deactive && extras.deactive.length && $.inArray(code, extras.deactive)>-1) {
                                            options.province = 0;
                                        } else {
